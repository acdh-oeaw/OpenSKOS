...
//			if ($i >= ($this->getFrom() + $this->getLimit())) break;
			...
			if ($dateSubmittedNodes->length > 0) {
				$data['created_timestamp'] = date(self::SOLR_DATETIME_FORMAT, strtotime($dateSubmittedNodes->item(0)->nodeValue));
			} else {
				$data['created_timestamp'] = date(self::SOLR_DATETIME_FORMAT);
			}
			$dateModifiedNodes = $xpath->query('dcterms:modified', $Description);
			if ($dateModifiedNodes->length > 0) {
				$data['modified_timestamp'] = date(self::SOLR_DATETIME_FORMAT, strtotime($dateModifiedNodes->item(0)->nodeValue));
			}
			$dateAcceptedNodes = $xpath->query('dcterms:dateAccepted', $Description);
			if ($dateAcceptedNodes->length > 0) {
				$data['approved_timestamp'] = date(self::SOLR_DATETIME_FORMAT, strtotime($dateAcceptedNodes->item(0)->nodeValue));
			}
			...
			if ($dateSubmittedNodes->length > 0 && ! $this->getOpt('ignoreIncomingStatus')) {
				if ($dateAcceptedNodes->length > 0) {
