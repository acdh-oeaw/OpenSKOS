...
class Api_Models_Concept implements Countable, ArrayAccess, Iterator
{
	...
	public function getNamespaces()
	{
		...
		$prefixes = array_merge($this['xmlns'], array('dc', 'dcterms', 'skos', 'openskos'));
	...
	public static function factory($data, Api_Models_Concepts $model = null)
	{
        ...
        // NOTE: changing SKOSCollection in getClass(Uri) to Collection creates unwanted side effects, so do it JIT in setConceptData
	...
	public function getConceptSchemes()
	{
        ...
        public function getSkosCollections()
	{
		$SkosCollections = array();
		foreach (self::$classes['SkosCollections'] as $subClass) {
			if (isset($this->data[$subClass])) {
				$SkosCollections[$subClass] = $this->data[$subClass];
			}
		}
		return $SkosCollections;
	}
	...
	public function save($extraData = null, $commit = null, $newCreated = null)
	{	
                $rdf = $this->toRDF()->documentElement->firstChild;
                $solrDocument = OpenSKOS_Rdf_Parser::DomNode2SolrDocument($rdf, $extraData);
		if ($newCreated) {
                        if (EPICHandleProxy::enabled()) {
                            ...
                            $uuid = current($solrDocument->offsetGet("uuid"));
                            try {
                                    $handleServerClient->createNewHandleWithGUID($forwardLocationPrefix . $uuid, $uuid);
                            } catch(Exception $ex) {
                                    throw new Zend_Controller_Action_Exception('Failed to create a PID for the new Object `'.$solrDocument['uri'][0].'`: '.$ex->getMessage(), 400);
                            }
                        }
		}
		$this->solr()->add($solrDocument, $commit);
     ...
     */
    public function setEPICHandle(array &$extraData)
    {
        // if EPIC is enabled and its a new concept
        if (EPICHandleProxy::enabled() && ( ! isset($extraData['uuid']) || empty($extraData['uuid'])))
        {
            $handleServerClient = EPICHandleProxy::getInstance();
            $this->data["uuid"] = $handleServerClient->spiceUUID($this["class"],$this->data["uuid"]);
            $extraData["uri"]   = $handleServerClient->getPID($this->data["uuid"]);
        }
    }
    ...
    public function setConceptData(array $formData, array $extraData = array())
    {
        ...
        // NOTE: changing SKOSCollection in getClass(Uri) to Collection creates unwanted side effects, so do it JIT
    	$rdfType->setAttribute('rdf:resource', preg_replace('/#SKOSCollection/','#Collection',$this->getClass()));
	...
	protected function getConceptSchemesData()
	{
		...
		if (isset($this['inScheme']) && is_array($this['inScheme'])) {
			$schemes = $apiClient->getConceptSchemes($this['inScheme'], $this->data['tenant']);
                ...
                if (isset($this['inSkosCollection']) && is_array($this['inSkosCollection'])) {
                    $this->updateSkosCollections(array(), $this['inSkosCollection']);
                }
	...
	/**
	 * Remove dependencies when a concept changes collections
	 * @param arary - uris of all new collections
	 */
	public function updateSkosCollections($newCollections, $oldCollections)
	{		
		foreach ($oldCollections as $collectionUri) {
			if (!in_array($collectionUri, $newCollections)) {
				$this->removeFromSkosCollection($collectionUri);
			}
		}
	}
	
	/**
	 * Removes a concept uri from SkosCollection resource fields.
	 * @param string $schemeUri
	 */
	public function removeFromSkosCollection($collectionUri)
	{
		$skosCollectionDocs = Editor_Models_ApiClient::factory()->getSkosCollections($collectionUri, $this['tenant']);
		if ( ! empty($skosCollectionDocs)) {
			$skosCollection = new Editor_Models_SkosCollection(new Api_Models_Concept(array_shift($skosCollectionDocs)));
			$skosCollection->removeTopConcept($this['uri']);
		}
	}
