...
class Editor_SearchController extends OpenSKOS_Controller_Editor
{
	public function indexAction()
	{
		...
		// Change search profile if needed
		$profileId = $this->getRequest()->getParam('searchProfileId', '');
		$userSearchOptions = $user->getSearchOptions();
		if ($userSearchOptions['searchProfileId'] != $profileId) {			
			$profilesModel = new OpenSKOS_Db_Table_SearchProfiles();
			$profile = $profilesModel->find($profileId)->current();
			if (null !== $profile) {
				$options = $profile->getSearchOptions();				
			} else {
				$options = Editor_Forms_SearchOptions::getDefaultSearchOptions();
			}
			$options['searchProfileId'] = $profileId;
			$user->setSearchOptions($options);
		}
		
		$model = new OpenSKOS_Db_Table_Users();
		$userForSearch = $model->find($this->view->searchForm->getValue('user'))->current();
		if (null === $userForSearch) {
			throw new Zend_Controller_Action_Exception('User not found', 404);
		}
		
		$detailedSearchOptions = $userForSearch->getSearchOptions($user['id'] != $userForSearch['id']);
		$allSearchOptions = $this->view->searchForm->getValues() + $detailedSearchOptions;
		
		$apiClient = new Editor_Models_ApiClient();
		
		try {
			$conceptsRaw = $apiClient->searchConcepts($allSearchOptions);
		} catch (Exception $ex) {
		...
		$this->getHelper('json')->sendJson(array('status' => 'ok', 'numFound' => $conceptsRaw['numFound'], 'concepts' => $concepts));
	}
	...
	public function setOptionsAction()
	{
		...
		if ((bool)$this->getRequest()->getParam('resetDefaults', false)) {
			if (isset($user['defaultSearchProfileId']) && ! empty($user['defaultSearchProfileId'])) {
				return $this->_forward('show-form', 'search', 'editor', array('searchProfileId' => $user['defaultSearchProfileId']));
			} else {
	...
	private function _getProfilesSelectOptions()
	{
		...
		$searchForm = new Editor_Forms_Search();
