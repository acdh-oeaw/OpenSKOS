...
class Editor_LoginController extends Zend_Controller_Action {
	...
	public function indexAction() {
		...
		if(!$shibbolethOptions || !$shibbolethOptions['enabled']) {
			$this->view->assign('shibbolethEnabled', false);
	  } else if(!isset($shibbolethOptions['authentication']) || !$shibbolethOptions['authentication']['eppn']) {
	  	$this->view->assign('shibbolethEnabled', false);
		} else {
			$this->view->assign('shibbolethEnabled', true);
			if(!isset($shibbolethOptions['authentication']['name']) || !is_string($shibbolethOptions['authentication']['name']) || !$shibbolethOptions['authentication']['name']) {
				$this->view->assign('shibbolethName', 'Shibboleth');
			} else {
				$this->view->assign('shibbolethName', $shibbolethOptions['authentication']['name']);
			}
		}
	...
	/**
	 * Starts an Shibboleth detection and login process.
	 *
	 */
	public function shibbolethLoginAction() {
		
		$form = Editor_Forms_ShibbolethLogin::getInstance();		
		if (!$eppn = $form->getShibbolethEppn()) {
			$this->render('shibboleth-login');
		} else {
			//check eppn
			$login = new Editor_Models_ShibbolethLogin();
			$login->setData($eppn);
			if ($login->isValid()) {
				$session = new Zend_Session_Namespace('Zend_Auth');
				// Set the time of user logged in
				...
				$emailform = $form->getShibbolethEmail();
				$this->view->assign('eppn', $eppn);
				$this->view->assign('emailform', $emailform);
				$this->render('shibboleth-login-form');
			  //send e-mail
			  if($emailform && isset($_POST["requestAccess"])) {
			  	if($form->sendShibbolethEmail($eppn, $this->getRequest()->getPost('request'))) {
			  	  $this->getHelper('FlashMessenger')->addMessage(_('Message sent to request access for '.$eppn));
			  	} else {
			  		$this->getHelper('FlashMessenger')->addMessage(_('An error occured while sending message to request access for '.$eppn));
			  	}  
				  $this->_helper->redirector('index', 'index');
			  }
			}  			
