...
/**
 * Holds the editor representation of a SkosCollection
 */
class Editor_Models_SkosCollection extends Api_Models_Concept 
{	
	/**
	 * Copy constructor.
	 * 
	...
	{
		parent::__construct(array_merge($copyFrom->getData(), array('class' => 'SKOSCollection')));
	}
	
	public function save($extraData = null, $commit = true, $createNew = false)
	{
		...
		*/
		...
		parent::save($extraData, $commit, $createNew);
	}
	...
	/**
	 * Delete a skos collection from everywhere.
	 * 
	...
	public function delete($commit = true, $deletedBy = null)
	{
		...
		$affectedConceptsQuery = '(inSkosCollection:"' . $this['uri'] . '" OR topConceptOf:"' . $this['uri'] . '") AND tenant:' . $this['tenant'];
		...
		do {
			...
			if (count($concepts['data']) > 0) {
				...
				foreach ($concepts['data'] as $key => $concept) {
					...
					if (isset($data['inSkosCollection'])) {
						$updateData['inSkosCollection'] = array_diff($data['inSkosCollection'], array($this['uri']));
					}
					if (isset($data['topConceptOf'])) {
						$updateData['topConceptOf'] = array_diff($data['topConceptOf'], array($this['uri']));
					}
					
					if (empty($updateData['inSkosCollection'])) {
	...
	protected function updateRelatedConcepts($includeConcepts)
	{
		...
		foreach ($newRelatedConcepts as $concept) {
			...
			if (! isset($conceptData['inSkosCollection'])) {
				$conceptData['inSkosCollection'] = array();
			}
			...
			$needsUpdate = false;
			if ( ! in_array($this['uri'], $conceptData['inSkosCollection'])) {
				$conceptData['inSkosCollection'][] = $this['uri'];
				$needsUpdate = true;
			}
		...
		foreach ($conceptsToRemove as $concept) {
			$conceptData = $concept->getData();
			$conceptData['inSkosCollection'] = array_diff($conceptData['inSkosCollection'], array($this['uri']));
	...
	{
		return new Editor_Models_SkosCollection(parent::factory($data, $model));
	}
}