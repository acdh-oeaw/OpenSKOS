...
class Editor_Models_Concept extends Api_Models_Concept 
{
	...
	{
		if ($ignoreValidation || $this->_validateSave()) {
	...
	protected function _getRelationToForm($relation, $schemeUuid, $schemeUri)
	{
		...
		$concepts = $this->getRelationsByField($relation, $schemeUri,  $callback);
		...
		foreach ($concepts as $concept) {
			...
			$previewLabel = $concept->getMlField('prefLabel', Zend_Registry::get('Zend_Locale')->getLanguage());
